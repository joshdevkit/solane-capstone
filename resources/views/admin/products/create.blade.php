        ...
        $('#add-row').click(function() {
            ...
            if (baseProductID === null && barcodeSymbology) {
                ...
                if (numericValue) {
                    ...
                    alert('Invalid symbology. Please enter a valid numeric value in barcode_symbology.');
                    return;
                }
            }

            const nextProductID = baseProductID !== null ? String(baseProductID + productCounter).padStart(3, '0') : '';

            const newRow = `
        <tr>
            <td class="p-2"><input type="text" name="product_id[]" class="p-2 w-full" value="${nextProductID}" readonly /></td>
            <td class="p-2"><input type="text" name="serial_no[]" class="p-2 w-full serial-no-input" /></td>
            <td class="p-2"><input type="text" name="net_weight[]" class="p-2 w-full" placeholder="Enter Net Weight" /></td>
            <td class="p-2"><input type="text" name="length[]" class="p-2 w-full" placeholder="Enter Length" /></td>
            <td class="p-2"><button class="remove-btn bg-red-500 text-white p-1 rounded-md hover:bg-red-600">Remove</button></td>
        </tr>
    `;
                ...
                success: function(response) {
                    ...
                    if (response.exists) {
                        ...
                        if (!$('#error-message').length) {
                            $('#barcode_symbology')
                                .after('<span id="error-message" class="text-red-500 text-sm">This product code already exists.</span>');
                        }
                    } else {
                        $('#barcode_symbology')
